	...
	public void actionPerformed(ActionEvent a) {
		if(a.getSource() == btnJoin) {
			...
			if(result == dao.SUCCESS) {
				jta.setText(cname+"님 가입 감사드립니다. \n 가입 축하 선물로 1000포인트를 드립니다.");
				reset();
			}
			...
			String ctel = txtPhone.getText().trim();
			if(ctel.equals("")) {
				jta.setText("전화번호를 입력하신 후 검색해주세요");
				return;
			} else if(ctel.length()<4) {
				jta.setText("전화번호는 4자리 이상 입력하셔야합니다");
				return;
			}
			...
			if(customer.size()>1) {
				...
				jta.setText("해당 번호로 등록된 회원이 없습니다");
				reset();
			}
			...
			if(customer.size()>0) {
				...
				jta.setText("데이터베이스에 등록된 회원이 없습니다");
			}
