...
-- 사원명, 직책, 입사일을 
-- 최초입사일부터 최근입사일 순으로 정렬하여 출력하는 SQL문을 작성하시오(5점).
SELECT ENAME, JOB, HIREDATE
    FROM EMP
    WHERE HIREDATE BETWEEN TO_DATE('1981년 2월 20일', 'YYYY"년"MM"월"DD"일"') AND TO_DATE('1981년 5월 1일', 'YYYY"년"MM"월"DD"일"')
    ORDER BY HIREDATE DESC;
    
-- 3. EMP테이블과 DEPT테이블의 self join과 outer join을 이용하여 
-- 사원명, 급여, 부서번호, 부서명, 근무지, 직속상사명을 
-- 상사가 없는 직원까지 전체 직원 다 출력하는 SQL문을 작성하시오. 단, 상사가 없는 경우 상사명 대신 ‘없음’을 출력한다(5점).
SELECT W.ENAME, W.SAL, W.DEPTNO, DNAME, LOC, NVL(M.ENAME, '없음') MANAGER
    FROM EMP W, EMP M, DEPT D
    WHERE W.MGR=M.EMPNO(+) AND W.DEPTNO=D.DEPTNO;
    
-- 4. SALGRADE 테이블의 LOSAL을 등급최저 급여, HISAL을 등급 최고 급여로 이용하여, EMP 테이블의 사원을 
-- 사번, 사원명, 직책, 급여, 급여등급을 급여 기준 내림차순으로 정렬하여 출력하는 SQL문을 작성하시오(5점).
...
-- 급여 앞에는 $를 붙이고, 숫자는 3자리마다 콤마( , )를 추가하여 출력하는 SQL문을 작성하시오. 
-- 단 급여(SAL)의 타입은 NUMBER(7,2)로 한다(5점).
SELECT EMPNO, ENAME, TO_CHAR(SAL, '$99,99.99') SAL
    FROM EMP
    WHERE DEPTNO = 20;
